...
/// Per-session mutex to serialize panel.json write operations.
...
fn build_panel_single(
    ...
    let prev_plan = read_existing_plan(panel_path, &bundle.full);
...
fn build_panel_multi(
    ...
    let prev_plan = read_existing_plan(panel_path, &combined);
...
/// Read the existing panel.json and return its plan if the diff matches.
fn read_existing_plan(
    panel_path: &std::path::Path,
...
/// Free per-session panel state when a PTY session is terminated.
pub fn cleanup_session_panel(session_id: &str) {
    if let Ok(mut map) = PANEL_LOCKS.lock() {
        map.remove(session_id);
    }
}

/// Returns true if an Anthropic API key is configured in env or ~/.atlas/config.json.
/// Retained so the panel store can surface configuration state in the UI.
#[tauri::command]
pub fn get_api_status() -> bool {
    if std::env::var("ANTHROPIC_AUTH_TOKEN").is_ok()
        || std::env::var("ANTHROPIC_API_KEY").is_ok()
    {
        return true;
    }
    crate::read_config_api_key().is_some()
}
